...
# sample_idx = np.where(np.argmax(y_test, axis=1) == 3)
sample_idx = np.random.choice(len(x_test), size=10, replace=False)
x_test = x_test[sample_idx][:10]
y_test = y_test[sample_idx][:10]
print(y_test)
...
attack = HopSkipJump(classifier=classifier, max_iter=10, targeted=False)
